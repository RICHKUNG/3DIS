...
def build_sam2_containment_tree(
    ...
    Note:
        Object IDs use level-prefixed format: L{level}{seq:03d}
        E.g., Level 2: 2001-2999, Level 4: 4001-4999, Level 6: 6001-6999
    """
    ...
    # Helper function to generate level-prefixed object IDs
    def make_global_id(level: int, local_id: int) -> int:
        """Convert local object ID to global level-prefixed ID.

        Format: L{level}{seq:03d}
        Examples: Level 2 obj 5 → 2005, Level 4 obj 123 → 4123

        Args:
            level: Hierarchy level (2, 4, 6, etc.)
            local_id: Sequential object ID within level (1, 2, 3, ...)

        Returns:
            Global ID with level prefix (2001-2999, 4001-4999, etc.)
        """
        if local_id > 999:
            LOGGER.warning(
                "Local ID %d exceeds 999 for level %d, may cause ID collision",
                local_id, level
            )
        return level * 1000 + local_id

    def parse_global_id(global_id: int) -> Tuple[int, int]:
        """Parse global ID back to (level, local_id).

        Args:
            global_id: Level-prefixed ID (e.g., 2005, 4123)

        Returns:
            (level, local_id) tuple
        """
        level = global_id // 1000
        local_id = global_id % 1000
        return level, local_id

    # Step 1: Load object frames for all levels
    LOGGER.info("Loading object metadata...")
    level_data: Dict[int, Dict[str, Any]] = {}
    # Mapping: local_id → global_id for each level
    ...
    for level in levels:
        ...
        local_to_global[level] = {}
        for local_id in object_frames.keys():
            global_id = make_global_id(level, local_id)
            local_to_global[level][local_id] = global_id

        LOGGER.info("Level %d: %d objects (IDs: %d-%d)",
                    level, len(object_frames),
                    make_global_id(level, 1),
                    make_global_id(level, len(object_frames)))
    ...
    for level in levels:
        ...
        for local_id, frames in object_frames.items():
            global_id = local_to_global[level][local_id]
            hierarchy[level][global_id] = {
                'parent': None,
                'children': [],
                'frames': frames,
                'local_id': local_id,  # Keep track of original ID for mask loading
            }
    ...
    for i, (coarse_level, fine_level) in enumerate(zip(levels[:-1], levels[1:])):
        ...
        # Load masks for both levels (using local IDs for file access)
        ...
        coarse_masks_local = load_masks_from_video_segments(
            coarse_dir,
            index_data=level_data[coarse_level]['index'],
        )

        LOGGER.info("Loading fine level (%d) masks...", fine_level)
        fine_masks_local = load_masks_from_video_segments(
            fine_dir,
            index_data=level_data[fine_level]['index'],
        )

        # Prepare parent candidates (use global IDs in hierarchy)
        parent_candidates = {}
        for parent_global_id in hierarchy[coarse_level]:
            parent_local_id = hierarchy[coarse_level][parent_global_id]['local_id']
            parent_candidates[parent_global_id] = {
                'frames': hierarchy[coarse_level][parent_global_id]['frames'],
                'masks': coarse_masks_local.get(parent_local_id, {}),
            }

        # Find parent for each fine-level object
        fine_objects = hierarchy[fine_level]
        for child_global_id in tqdm(fine_objects, desc=f"L{coarse_level}→L{fine_level}"):
            child_local_id = fine_objects[child_global_id]['local_id']
            child_frames = fine_objects[child_global_id]['frames']
            child_masks_dict = fine_masks_local.get(child_local_id, {})

            result = find_best_parent(
                child_id=child_global_id,  # Log with global ID
            ...
            if result is not None:
                parent_global_id, containment = result
                fine_objects[child_global_id]['parent'] = parent_global_id
                fine_objects[child_global_id]['parent_containment'] = containment

                # Add to parent's children list
                hierarchy[coarse_level][parent_global_id]['children'].append(child_global_id)

    # Step 4: Build ancestor paths
    LOGGER.info("Building ancestor paths...")
    paths: Dict[int, List[int]] = {}

    def build_path(global_id: int, level: int) -> List[int]:
        """Recursively build path from root to object using global IDs."""
        if level not in hierarchy or global_id not in hierarchy[level]:
            return [global_id]

        parent_global_id = hierarchy[level][global_id].get('parent')
        if parent_global_id is None:
            return [global_id]

        # Find parent's level (always coarser, earlier in the list)
        parent_level = None
        for lv in sorted(levels):
            if lv >= level:
                break
            if lv in hierarchy and parent_global_id in hierarchy[lv]:
                parent_level = lv
                break
        ...
        parent_path = build_path(parent_global_id, parent_level)
        return parent_path + [global_id]

    for level in levels:
        for global_id in hierarchy[level]:
            paths[global_id] = build_path(global_id, level)
    ...
    # Clean up hierarchy for serialization (remove frames/masks/local_id, convert sets)
    clean_hierarchy = {}
    for level, level_objs in hierarchy.items():
        clean_hierarchy[str(level)] = {}
        for global_id, obj_data in level_objs.items():
            clean_hierarchy[str(level)][str(global_id)] = {
                'parent': obj_data['parent'],
                'children': obj_data['children'],
                'descendant_count': len([
                    oid for oid in paths
                    if global_id in paths[oid] and oid != global_id
                ])
            }
            # Include parent_containment if available
            if 'parent_containment' in obj_data:
                clean_hierarchy[str(level)][str(global_id)]['parent_containment'] = (

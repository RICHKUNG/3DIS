...
| `hierarchical.py` | `HierarchicalStrategy.__init__`, `infer`, `_coarse_localization`, `_refine_objects`, `_search_parts_for_objects`, `_get_part_candidates_for_object`, 以及對應的 `_..._with_combined_feature` | **[2025-11-19 更新]** 依據家族樹自粗到細搜尋，並支援混合查詢管線。每個 `_` 函數皆與 `family_tree_query.FamilyTreeQuery` 互動以取得親子節點。**重要修復**：移除所有硬編碼的 L2/L4/L6 層級，現使用動態檢測的 `self.coarse_level`、`self.refinement_level`、`self.part_levels`，支持任意層級組合（如 L1/L3/L5）。 |
